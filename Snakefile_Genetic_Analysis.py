...
rule Index_VCF_File:
    input:
        VCF_FILE='{sample}.vcf.bgz' ############ Will need change name here to work with unzipped VCFs
    output:
        INDEXED_VCF_FILE='{sample}.vcf.bgz.csi'
    threads:8
    run:
...
######################################################################################################################
#### Step 2 - Filter VCF for SNPs within the boundaries of Genes of interest
...
rule Prepare_for_second_filtering:
    ...
    output:
        ...
        GENE_FILTERED_GZVCF_INDEX='{sample}_Gene_Filtered.vcf.gz.csi'
    threads:8
    run:
